...
| KDPII test — FrameByFrame/privacy-filter-korean 과 비교 | 4,891 | **0.9339** (그 모델 9라벨 한정 **0.9383**) | — | FrameByFrame 0.5156 (9라벨 한정 0.6824) | **+41.8pt** |
| BCCard validation · ko | 10,743 | **0.9826** | 0.9824 | 0.9594 | **+2.3pt** |
| 합성 heldout v2 | 670 | **0.9652** | — | 0.5328 (29라벨 한정 0.5505) | **+43.2pt** |
...
디코더 수정(2026-09-17): 이름 끝 '아/야' 호격 조사 규칙에 받침 조건을 추가(`최수아→최수` 절단 버그). 같은 후처리를 쓰는 Veil·BCCard 값은 수정 후 재측정치이며(변화 ≤ +0.1pt), Azure 값은 재호출하지 않아 수정 전 후처리 기준이다(영향 정답 스팬 KDPII 1/274, BCCard val 30/11,417).
